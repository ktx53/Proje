        ...
        if(secim){
          ...
          if(sira < sunucudanGelen.sorular.length) {
              ...
              goruntulemeAlani.innerHTML = `
                  <h2>Mevcut sorulardan ${puan}/${sunucudanGelen.sorular.length} Yatırım yapmaya hazır değilsiniz</h2>
  
                  <button onclick="location.reload()">Yeniden Başla</button>
              `}
              else{
                goruntulemeAlani.innerHTML = `
                <h2>Mevcut sorulardan ${puan}/${sunucudanGelen.sorular.length} Yatırım yapmaya hazırsınız</h2>
                <button onclick="location.reload()">Yeniden Başla</button>`
              }
